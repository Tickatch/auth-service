 ...
 *
 * <p>소셜 로그인, 계정 연동/해제를 담당한다. OAuth는 CUSTOMER 전용이다.
 *
 * @author Tickatch
 * @since 1.0.0
 */
...
public class OAuthCommandService {
   ...
   * @return 인증 URL
   */
  public String getAuthorizationUrl(ProviderType providerType, boolean rememberMe, String deviceInfo) {
    // 제공자 설정 확인
    if (!oAuthPort.isProviderConfigured(providerType)) {
      throw new AuthException(AuthErrorCode.OAUTH_PROVIDER_NOT_CONFIGURED);
    }

    // 상태값 생성 (CSRF 방지 + 추가 정보)
   ...
   * 일반 로그인 콜백 처리.
   */
  private LoginResult handleLoginCallback(ProviderType providerType, String code, OAuthState state) {
    // 사용자 정보 조회
    ...
    // 기존 계정 확인 (제공자 정보로)
    Auth auth = authRepository.findByProviderAndProviderUserId(providerType, userInfo.providerUserId())
        .orElse(null);
    ...
    auth = authRepository.findByEmailAndUserType(userInfo.email(), UserType.CUSTOMER)
        .orElse(null);
  ...
  private LoginResult handleLinkCallback(ProviderType providerType, String code, OAuthState state) {
    // Auth 조회
    Auth auth = authRepository.findById(state.linkAuthId())
        .orElseThrow(() -> new AuthException(AuthErrorCode.AUTH_NOT_FOUND));
    ...
    authRepository.findByProviderAndProviderUserId(providerType, userInfo.providerUserId())
  ...
  private LoginResult registerNewAccount(OAuthUserInfo userInfo, OAuthState state) {
    // OAuth 회원은 임시 비밀번호 생성 (나중에 변경 유도)
    String tempPassword = UUID.randomUUID().toString();

    Auth auth = Auth.registerWithOAuth(
        userInfo.email(),
        tempPassword,
        UserType.CUSTOMER,  // OAuth는 CUSTOMER 고정
    ...
    log.info("OAuth 회원가입 완료 - authId: {}, email: {}, provider: {}",
        auth.getId(), auth.getEmail(), userInfo.providerType());
